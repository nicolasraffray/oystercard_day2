...
describe Oystercard do
  ...
  describe "#initialize" do
    ...
    it "initializes a new JourneyLog object" do
  ...
  describe "#touch_in" do
    ...
    # it "saves station" do
    #   expect(subject.instance_variable_get(:@current_journey)).to receive(:start).with(station)
    #   subject.touch_in(station)
    # end
  end
  ...
  describe "#touch_out" do
    ...
    it "deducts fare if all good" do
      subject.touch_in(station)
      expect{subject.touch_out(exit_station)}.to change{ subject.balance}.by(-Oystercard::MIN)
    end

    it 'returns pen if touch out but no touch in' do
      expect{subject.touch_out(exit_station)}.to change{ subject.balance}.by(-Oystercard::PENALTYFARE)
    end

    # it "tells current_journey exit station" do
    #   expect(subject.instance_variable_get(:@current_journey)).to receive(:finish).with(exit_station)
    #   subject.touch_out(exit_station)
    # end
  end
end
